...
WORKDIR /src
COPY ["CollegeApi.csproj", "."]
RUN dotnet restore "./CollegeApi.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./CollegeApi.csproj" -c %BUILD_CONFIGURATION% -o /app/build

# Этот этап используется для публикации проекта службы, который будет скопирован на последний этап
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./CollegeApi.csproj" -c %BUILD_CONFIGURATION% -o /app/publish /p:UseAppHost=false
